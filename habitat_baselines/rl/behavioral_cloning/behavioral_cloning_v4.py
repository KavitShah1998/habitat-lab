...
class BehavioralCloning(BaseRLTrainer):
    ...
    def train(self):
        ...
        while True:
            ...
            for _ in BATCH_LENGTH:
                in_batch.append(copy_batch(batch, device=self.device))
                in_hidden.append(student_hidden_states.detach().clone())
                in_prev_actions.append(student_prev_actions.detach().clone())
                in_not_done.append(not_done_masks.detach().clone())
                ...
                out_teacher_actions.append(teacher_actions_converted)
                ...
                for idx, done in enumerate(dones):
                    if done:
                        ...
                        suc_deq.append(infos[idx]['success'])
            ...
            in_batch = cat_batch(in_batch)
            in_hidden = torch.cat(hidden_state_buff, dim=1)
            in_prev_actions = torch.cat(prev_actions_buff, dim=0)
            in_not_done = torch.cat(not_done_buff, dim=0)
            out_teacher_actions = torch.cat(out_teacher_actions, dim=0)
            (
                _,
                out_student_actions,
                _,
            ...
            # Loss and update
            student_actions_tanh = torch.tanh(out_student_actions)
            action_loss = (
                student_actions_tanh - out_teacher_actions
            ).pow(2).mean()
